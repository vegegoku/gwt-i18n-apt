 ...
 * the License.
 */
package org.gwtproject.i18n.processor.logging.logger;
...
public abstract class TreeLogger {
  ...
  /**
   * Provides extra information to the user, generally details of what caused the problem or what
   * the user should do to fix the problem. How this information is interpreted and displayed is
  ...
  public enum Type {
    ...
    /**
     * Indicates whether this severity type represents a high severity that should be highlighted
     * for the user.
     *
     * @return <code>true</code> if this severity is high, otherwise <code>false</code>.
     */
   ...
   * null</code> <code>caught</code> and <code>helpInfo</code>.
   */
  ...
  /**
   * Produces a branched logger, which can be used to write messages that are logically grouped
   ...
   * use this method to avoid constructing log messages that would be thrown away.
   */
  public abstract boolean isLoggable(TreeLogger.Type type);

  /**
   * Calls {@link #log(TreeLogger.Type, String, Throwable, HelpInfo)} with a <code>null</code>
   * <code>caught</code> and <code>helpInfo</code>.
   */
  ...
  /**
   * Logs a message and/or an exception, with optional help info. It is also legal to call this
   * method using <code>null</code> arguments for <i>both</i> <code>msg</code> and <code>caught
   * </code>, in which case the log event can be ignored. The <code>info</code> can provide extra
   * information to the logger; a logger may choose to ignore this info.
   *
   * @param type
   * @param msg an optional message to log, which can be <code>null</code> if only an exception is
   *     being logged
